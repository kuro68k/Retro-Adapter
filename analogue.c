...
void ReadAnalogue(report_t *reportBuffer, uchar id)
{
	...
	if (((PIND & (1<<0)) == 0) && ((PIND & (1<<3)) == 0))	// BitStik
		bitstik = 1;
	...
	if (id == 1)
	{
		...
		if (bitstik) {						// axis inverted
			reportBuffer->x = 0-reportBuffer->x;
			reportBuffer->y = 0-reportBuffer->y;
		}
	}
	...
	{
		if (bitstik) {							// BitStik
			ADMUX	= (1<<REFS0)|(1<<ADLAR)|1;		// PC1/ADC1 - Z
			ADCSRA	|= (1<<ADSC);					// start conversion
			while (ADCSRA & (1<<ADSC));				// wait for conversion to finish
			reportBuffer->rx = 127 + ADCH;
		} else {								// high = 2 button, 4 axis mode
			hidMode = HIDM_2P;
	...
	{
		if (bitstik) {
			if (!(PINB & (1<<1))) reportBuffer->b1 |= (1<<0);
			if (!(PINC & (1<<0))) reportBuffer->b1 |= (1<<1);
			if (!(PIND & (1<<4))) reportBuffer->b1 |= (1<<2);
		} else {
			if (!(PIND & (1<<4))) reportBuffer->b1 |= (1<<0);
			if (!(PINB & (1<<1))) reportBuffer->b1 |= (1<<0);
		}
	}
	else
